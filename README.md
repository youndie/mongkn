...
`deleteOne`, `countDocuments`, `find`), 39 тестов — включая дифференциальные против
официального драйвера, стресс-тесты пула соединений и проверку утечек через подменённый
аллокатор libbson. Форма API снята с официального `mongodb-driver-kotlin-coroutine`, а совпадение с ним проверяется
