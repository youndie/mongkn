...
`deleteOne`, `countDocuments`, `find`), 35 тестов против настоящего mongod — включая
дифференциальные против официального драйвера и стресс-тесты пула соединений. Форма API снята с официального `mongodb-driver-kotlin-coroutine`, а совпадение с ним проверяется
