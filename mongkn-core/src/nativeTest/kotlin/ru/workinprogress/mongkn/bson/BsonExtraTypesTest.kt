...
class BsonExtraTypesTest {
    ...
    fun `binary subtype is part of the value`() {
        ...
        assertEquals(BsonBinary.UUID, (roundTrip(BsonDocument("id" to BsonBinary(BsonBinary.UUID, bytes)))["id"] as BsonBinary).subtype)
    }
    ...
    fun `the two deprecated types are still refused loudly`() {
        ...
        val failure = assertFailsWith<UnsupportedBsonTypeException> {
