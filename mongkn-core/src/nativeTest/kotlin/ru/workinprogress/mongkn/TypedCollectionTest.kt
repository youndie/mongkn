...
class TypedCollectionTest {
    ...
    @Test
    fun `data class survives a real round trip`() =
        runTest {
    ...
    @Test
    fun `typed and untyped views agree on the stored document`() =
        runTest {
            val client = connect()
    ...
    @Test
    fun `a document written untyped reads back as a class`() =
        runTest {
            val client = connect()
    ...
    @Test
    fun `filters stay untyped documents`() =
        runTest {
    ...
    @Test
    fun `ObjectId in a class stays an ObjectId in the database`() =
        runTest {
            val client = connect()
            ...
            assertEquals(
                ...
                client
                    .getDatabase(DATABASE)
                    .getCollection<WithId>(collectionName)
                    .find()
                    .first(),
            )
        }
