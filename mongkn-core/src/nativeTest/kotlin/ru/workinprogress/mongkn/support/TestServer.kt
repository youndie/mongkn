...
object TestServer {
    ...
    /**
     * Адрес **mongos** — точки входа в шардированный кластер (M-66).
     *
     * Шарды и конфигурационный реплика-сет своих адресов здесь не имеют, и это не упущение:
     * драйвер к ним не обращается никогда. Клиент видит только mongos, а куда идти дальше,
     * решает уже он — потому шардированный кластер и стоит в контуре одним адресом,
     * хотя процессов за ним четыре.
     */
    val shardHost: String =
        getenv("MONGKN_TEST_SHARD_HOST")?.toKString()?.takeIf { it.isNotBlank() } ?: "127.0.0.1:27021"
    ...
    /**
     * Строка подключения к шардированному кластеру.
     *
     * `replicaSet` здесь не указывается **никогда**: по этому параметру драйвер определяет
     * топологию как реплика-сет и к mongos пойдёт не тем протоколом. Топологию `Sharded`
     * он выводит сам — из ответа `hello`, в котором mongos представляется `isdbgrid`.
     *
     * @param options параметры строки подключения без ведущего `?`.
     */
    fun shardUri(options: String = ""): String = "mongodb://$shardHost" + if (options.isEmpty()) "" else "/?$options"
