...
public class MongoClient(
    ...
    public companion object {
         ...
         *
         * ## На многоядерном сервере задавайте [ioThreads] явно
         *
         * Колено кривой **едет вместе с машиной**, а умолчание — константа. Замер выше снят
         * на восьмиядерной macOS; на двадцатиядерном Linux колено на **64**, и переход 32 → 64
         * там даёт не полтора процента, а **+70 %** при вдвое более высоком потолке
         * (~20 000 оп/с против ~10 600). До 32 потоков обе кривые почти совпадают — расходятся
         * ровно там, где на слабой машине рост уже кончился.
         *
         * Умолчанием всё же оставлены 32, и вот цена вопроса (Linux, 256 одновременных вставок,
         * два прогона на настройку): 32 потока — 93 МБ RSS, 64 потока — **156 МБ**, то есть
         * +68 % памяти, около 2.2 МБ на поток. В покое разницы нет вовсе (21.9 МБ при обеих
         * настройках): потоки заводятся лениво, и платит только тот, кто дошёл до такой
         * одновременности.
         *
         * Отсюда и выбор: выигрыш привязан к машине, а расход — к нагрузке, и возникает он там,
         * где память считают, — в контейнере с лимитом. Ошибка «медленнее, чем могло бы» чинится
         * этим параметром при первом же профилировании; ошибка «съел лимит контейнера» чинится
         * разбором инцидента. Если у вас двадцать ядер и высокая одновременность — ставьте
         * `ioThreads = 64` осознанно, померив память своего сервиса.
         *
