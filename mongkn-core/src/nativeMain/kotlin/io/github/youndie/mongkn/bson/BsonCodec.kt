package io.github.youndie.mongkn.bson
...
@OptIn(ExperimentalForeignApi::class)
internal fun CPointer<bson_t>.toDocument(): BsonDocument =
    memScoped {
...
@OptIn(ExperimentalForeignApi::class)
private fun MemScope.appendEntries(
    target: CPointer<bson_t>,
...
@OptIn(ExperimentalForeignApi::class)
private fun MemScope.appendValue(
    target: CPointer<bson_t>,
    key: String,
    value: BsonValue,
) {
    ...
    val ok =
        when (value) {
            is BsonString -> {
                appendUtf8(target, key, value.value)
            }

            is BsonInt32 -> {
                bson_append_int32(target, key, -1, value.value)
            }

            is BsonInt64 -> {
                bson_append_int64(target, key, -1, value.value)
            }

            is BsonDouble -> {
                bson_append_double(target, key, -1, value.value)
            }

            is BsonBoolean -> {
                bson_append_bool(target, key, -1, value.value)
            }

            BsonNull -> {
                bson_append_null(target, key, -1)
            }

            is BsonDateTime -> {
                bson_append_date_time(target, key, -1, value.epochMillis)
            }

            is BsonTimestamp -> {
                bson_append_timestamp(target, key, -1, value.seconds, value.increment)
            }

            is BsonRegex -> {
                bson_append_regex(target, key, -1, value.pattern, value.options)
            }

            // У bson_append_code нет параметра длины — NUL внутри кода непредставим средствами
            // самой libbson, не только нашими.
            is BsonCode -> {
                bson_append_code(target, key, -1, value.code)
            }

            is BsonSymbol -> {
                bson_append_symbol(target, key, -1, value.value, -1)
            }

            BsonUndefined -> {
                bson_append_undefined(target, key, -1)
            }

            BsonMinKey -> {
                bson_append_minkey(target, key, -1)
            }

            BsonMaxKey -> {
                bson_append_maxkey(target, key, -1)
            }
            ...
            is BsonArray -> {
                withChild(value.values.mapIndexed { i, v -> i.toString() to v }) { child ->
 ...
 */
@OptIn(ExperimentalForeignApi::class)
internal fun canonicalDecimal128(text: String): String =
    memScoped {
        val decimal = alloc<bson_decimal128_t>()
 ...
 */
@OptIn(ExperimentalForeignApi::class)
private fun MemScope.appendUtf8(
    target: CPointer<bson_t>,
    key: String,
    value: String,
): Boolean {
...
internal fun MemScope.readValue(
    ...
    when (val type = bson_iter_type(iter)) {
        ...
        BSON_TYPE_INT32 -> {
            BsonInt32(bson_iter_int32(iter))
        }

        BSON_TYPE_INT64 -> {
            BsonInt64(bson_iter_int64(iter))
        }

        BSON_TYPE_DOUBLE -> {
            BsonDouble(bson_iter_double(iter))
        }

        BSON_TYPE_BOOL -> {
            BsonBoolean(bson_iter_bool(iter))
        }
        ...
        BSON_TYPE_DATE_TIME -> {
            BsonDateTime(bson_iter_date_time(iter))
        }
        ...
        BSON_TYPE_DOCUMENT -> {
            BsonDocument(readEntries(recurse(iter)))
        }

        // Массив читается тем же обходом, просто ключи "0", "1", … выбрасываются.
        BSON_TYPE_ARRAY -> {
            BsonArray(readEntries(recurse(iter)).map { it.second })
        }

        else -> {
            throw UnsupportedBsonTypeException(type, key)
        }
    }
