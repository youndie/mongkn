...
**Следствие.** Фаза 1 драфта закрыта, но с поправкой из 1.1. Живой код — [Mongkn.kt](../../mongkn-core/src/nativeMain/kotlin/io/github/youndie/mongkn/Mongkn.kt).
...
[MongoException](../../mongkn-core/src/nativeMain/kotlin/io/github/youndie/mongkn/MongoException.kt)
...
**Как решено.** [MongoClient](../../mongkn-core/src/nativeMain/kotlin/io/github/youndie/mongkn/MongoClient.kt)
...
| Факт | Где проверено |
|---|---|
| `mongoc_client_pool_max_size(pool, n)` задаёт размер пула; вызывается до первого `pop` | `mongoc/mongoc-client-pool.h:60`, вызов в [MongoClient](../../mongkn-core/src/nativeMain/kotlin/io/github/youndie/mongkn/MongoClient.kt) |
...
| `staticCFunction` заполняет все пять; счётчик держится в top-level `AtomicLong`, потому что захват контекста в `staticCFunction` недоступен | [BsonAllocations](../../mongkn-core/src/nativeTest/kotlin/io/github/youndie/mongkn/bson/BsonAllocations.kt) |
...
[BsonRoundTripTest](../../mongkn-core/src/nativeTest/kotlin/io/github/youndie/mongkn/bson/BsonRoundTripTest.kt)
...
| Файлы читаются libbson напрямую (`bson_json_reader_new_from_file`) — парсер тащить не надо | [SpecTestRunner](../../mongkn-core/src/nativeTest/kotlin/io/github/youndie/mongkn/spec/SpecTestRunner.kt) |
...
### Р1а. Минимальная версия libmongoc — 1.26

Решение: поддерживаем **1.26 и новее**, целимся по-прежнему в 2.x.

Почему именно 1.26, а не что-то новее:

- это Ubuntu 24.04 LTS с поддержкой до 2029 — самый распространённый серверный дистрибутив;
- ветка 1.x на Linux **актуальна**: 2.x из пакетов не ставится нигде, только сборка
  из исходников (Ubuntu 25.04 — 1.30.2, Debian 13 — 1.30.4);
- цена посчитана сверкой заголовков, а не оценена: 1.30 добавляет 103 функции и не удаляет
  ни одной; из них 40 — новый клиентский `mongoc_bulkwrite_*`, 18 — структурированное
  логирование, остальное по мелочи. Отказываемся ровно от трёх возможностей, ни одна
  не блокирует (см. M-81 бэклога).

Проверка границы не требует отдельной работы: Linux-джоба CI идёт против 1.26, macOS —
против 2.x, поэтому каждый прогон проверяет и пол, и потолок.

**Ловушка, стоившая ложного вывода.** Дважды в ходе M-64 и M-79 было записано «этого нет
в ветке 1.x», тогда как на деле «этого нет в 1.26»: обобщение с одной установленной версии
на всю ветку без проверки. Отсюда правило — отсутствие функции проверять на **той версии,
которую объявляем минимальной**, а не на той, что оказалась под рукой.
...
[MongoCollection](../../mongkn-core/src/nativeMain/kotlin/io/github/youndie/mongkn/MongoCollection.kt)
...
**Проверено (M-68).** Утверждение перестало быть гипотезой: сервер держит команду две секунды
(failpoint `blockConnection`), корутина отменяется через 300 мс, и `withTimeout` возвращает
управление **через две секунды**, а не через триста миллисекунд. Тест `FailureModesTest`
фиксирует это, чтобы изменение поведения было замечено.

Хорошая часть: клиент при этом **возвращается в пул**. Отдельный тест берёт клиентов больше,
чем разрешений семафора, после отменённой операции — если бы отменённая не вернула своего,
прогон встал бы навсегда. То есть неотменяемость вызова стоит задержки, а не утечки.
