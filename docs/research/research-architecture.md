...
**Состояние на момент записи: решение принято, код не удалён.** Генератор всё ещё в сборке
и всё ещё печатает `MongoCollection`; удаление — задача M-33.
...
**Риск 5. Ресурсная модель (Р2) не покрыта ни одним тестом.** Пул вместо одного клиента заведён
потому, что `mongoc_client_t` не потокобезопасен, — но все 25 тестов последовательные, и сотню
корутин на одном `MongoClient` мы не запускали ни разу. Ни spec-, ни дифференциальные тесты сюда
не дотягиваются: они про поведение, а здесь UB под нагрузкой. Смягчение: конкурентный
стресс-тест, задача M-29. Это самая большая непроверенная область проекта.
