...
Знаменатель проверен по делу, а не по документации: список методов снят с jar официального
драйвера (`javap` по `com.mongodb.kotlin.client.coroutine.MongoCollection`), из него убраны
геттеры, настройки `with*` и перегрузки одной операции — остаётся ровно 30. Не реализованы
шесть, и все шесть намеренно: пять — поисковые индексы **Atlas**, которых на своём сервере нет
вовсе, шестая — устаревший `mapReduce`. Обычных операций в пропусках нет.

`dropIndexByKeys` в mongkn отдельная функция, а у драйвера — перегрузка `dropIndex`, поэтому
публичных функций у нас 25, а покрытых операций драйвера 24.
...
| Проверки | 294 теста на macOS, 277 из них и на Linux, все зелёные на обеих; ABI-валидация на публикуемой платформе (linuxX64), ktlint в гейте; инсценированные сбои адресованы своему клиенту по `appName` (M-82) |
