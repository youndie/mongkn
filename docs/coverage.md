...
| Аутентификация | SCRAM проверен на отдельном сервере с `--auth`; TLS и x509 — нет (M-70) |
...
транзакция не перезапускается сама — см. M-68 и M-69.
...
а SCRAM-аутентификация покрыта. Остаются TLS и x509, шардированный кластер и производительность —
её не измеряли вовсе. Код там может работать, а может и нет;
