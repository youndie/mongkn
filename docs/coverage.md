...
* **Мониторинг команд (APM)** — из-за этого 14 официальных spec-сценариев пропускаются (M-79),
  и это **единственная** оставшаяся причина пропусков. Логи драйвера при этом есть ([MongknLog]) —
  это другой механизм: сообщения libmongoc, а не события команд.
...
| Диагностика | обработчик логов драйвера ([MongknLog]); мониторинг команд (APM) — нет |
...
| Проверки | 198 тестов на двух платформах, ABI-валидация, ktlint в гейте |
