...
| Проверка настоящим потребителем | слой доступа к данным внутренний сервис переписан на mongkn (M-86). Нашлось четыре **тихих** дефекта — тех, что не падают, а молча возвращают пустоту; внутренние проверки их не видели, потому что ловят падения |
