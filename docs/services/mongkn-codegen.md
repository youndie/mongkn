...
> **Модули уходят.** Решением [Р9](../research/research-architecture.md) от генерации API
> отказались: замер (§1.10) показал одну сгенерированную строку на шесть рукописных, а главное —
> генератор гарантирует *форму* API, а не *поведение*, то есть отвечает не на тот вопрос.
> Удаление — задача M-33.
>
> **Пока они на месте и работают:** `MongoCollection` всё ещё генерируется, и `:mongkn-core`
> без этих модулей не собирается. Документ описывает то, что есть, а не то, что будет.
>
> Официальный JVM-драйвер из проекта при этом **не уходит** — он остаётся эталоном для
> дифференциальных тестов (M-28). Рассуждение в решении Р5 о том, что обращаться к нему можно
> только с JVM, остаётся верным и работает уже на них.
