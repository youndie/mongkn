...
| Файл | Что там |
|---|---|
| [ReferenceDocument.kt](../../mongkn-difftest/src/main/kotlin/io/github/youndie/mongkn/difftest/ReferenceDocument.kt) | эталонный документ: по полю на каждый поддержанный тип BSON |
| [Seed.kt](../../mongkn-difftest/src/main/kotlin/io/github/youndie/mongkn/difftest/Seed.kt) | фаза A |
| [Verify.kt](../../mongkn-difftest/src/main/kotlin/io/github/youndie/mongkn/difftest/Verify.kt) | фаза C, расхождения печатаются по полям |
| [MongoDifferentialTest.kt](../../mongkn-core/src/nativeTest/kotlin/io/github/youndie/mongkn/MongoDifferentialTest.kt) | фаза B |
...
* **Все три фазы помечены `upToDateWhen { false }`**, включая нативный тест. Причина не
  в перестраховке: фаза A всегда чистит коллекции, а нативная задача при инкрементальном прогоне
  может оказаться `UP-TO-DATE` и не отработать — тогда фаза C падает на пустой `written`.
  Ровно это и случилось при первом прогоне после удаления генератора. Корректность здесь зависит
  от состояния mongod, а его Gradle не отслеживает, — значит кэшировать нечего.
  Цена: нативные тесты всегда перезапускаются.
