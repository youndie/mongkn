...
> **Меняется.** По решению Р9 генерация уходит, `MongoCollection` вернётся в рукописный вид
> (задача M-33). Ниже описано текущее устройство. **Важное следствие для этого документа:**
> знание о переводе типов перестаёт быть исполняемым и остаётся только здесь, в разделе 3, —
> то есть теперь его протухание ничем не ловится, кроме дифференциальных тестов (M-28).
...
Важно не переоценивать эту проверку: она про **форму**. Что `updateOne` действительно обновляет
ровно один документ и правильно различает `matchedCount` и `modifiedCount`, компиляция не знает —
это вопрос поведения, и отвечают на него тесты вехи M6.
