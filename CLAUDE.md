...
1. [docs/research/research-architecture.md](docs/research/research-architecture.md) — решения
   ...
   - генератор живёт в JVM-модулях (`:mongkn-codegen` + `:mongkn-api-spec`), а не в нативном:
     ...
     **генерируется** — исходника в репозитории нет, правки идут в процессор или
     в `CollectionOps`.
2. [BACKLOG.md](BACKLOG.md) — что делать. Закрыты вехи M0–M5а; дальше M6 (выпуск, CI)
   и M7 (типизированные коллекции и infix-DSL). Открытый вопрос M-27: оставлять ли генератор —
   замер (§1.10) показал, что покрытие он не удешевляет.
